			...
			<view class="uni-list-item__container" :class="{ 'container--right': showArrow || link, 'flex--direction': direction === 'column' }">
				...
				<slot name="body">
					<view class="uni-list-item__content" :class="{ 'uni-list-item__content--center': thumb || showExtraIcon || showBadge || showSwitch }">
						<text v-if="title" class="uni-list-item__content-title" :class="[ellipsis !== 0 && ellipsis <= 2 ? 'uni-ellipsis-' + ellipsis : '']"><text v-if="point" class="uni-red-cc">*</text>{{ title }}</text>
						<text v-if="note" class="uni-list-item__content-note" v-html="note"></text>
					</view>
				</slot>
				<slot name="footer">
